...
let PROTOCOL = "https://cdn.jsdelivr.net/gh/aslushnikov/devops.aslushnikov.com@browser-protocols-data/firefox_protocol.js";
...
class App {
  ...
  async _initialize() {
    const ffProtocol = await fetch(PROTOCOL).then(r => r.text());
    const cdpDomains = ffProtocolToCDP(ffProtocol);

    this._allDomains.clear();

    const protocol = App._normalizeProtocol({domains: cdpDomains});
